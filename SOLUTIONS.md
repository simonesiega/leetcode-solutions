...
| 392 | [Is Subsequence](https://leetcode.com/problems/is-subsequence/) | [392.py](neetcode-all/arrays-and-hashing/392.py) | `O(n)`, where `n = len(t)`; the implementation scans `t` once. | `O(1)`; only the current index in `s` is stored. | ![Easy](https://img.shields.io/badge/Easy-1f883d?style=flat-square) | 2 |
